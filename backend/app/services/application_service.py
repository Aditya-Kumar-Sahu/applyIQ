...
class ApplicationService:
    async def list_applications(self, *, session: AsyncSession, user: User) -> ApplicationsListData:
        log_debug(logger, "applications.list.start", user_id=user.id)
        try:
            ...
            log_debug(logger, "applications.list.complete", user_id=user.id, items_count=len(items))
    ...
    async def get_stats(self, *, session: AsyncSession, user: User) -> ApplicationsStatsData:
        log_debug(logger, "applications.stats.start", user_id=user.id)
        try:
            applications = list(await session.scalars(select(Application).where(Application.user_id == user.id)))
            if len(applications) == 0:
                log_debug(logger, "applications.stats.empty", user_id=user.id)
            ...
            log_exception(logger, "applications.stats.failed", error, user_id=user.id)
            raise
